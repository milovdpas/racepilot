...
- **Top gotchas that will bite you:**
  ...
  - Persisted-shape changes require **bumping the persist `version` + an additive `migrate`** in `use-training-store.ts` (currently v19).
  ...
  - **Manifest icons must be PNG.** An SVG entry with `sizes: "any"` looks like a perfect match to Chrome's icon picker, but it cannot rasterize one for an Android WebAPK — it then reports `no-acceptable-icon` and stops looking, and "Install" silently degrades to a home-screen shortcut. `app/icon.svg` is the tab favicon only; `public/icons/*.png` (regenerate with `scripts/build-icons.mjs`) is what `app/manifest.ts` points at. There are four sets, one per app mark, and the manifest picks by the `rp_mark` cookie so a swimmer installing the app does not get a runner. That choice is only possible **at install time**: Android and iOS both capture the icon then and never revisit it. The in-app badge is `components/layout/app-mark.tsx`; keep its artwork in step with `scripts/build-icons.mjs`.
